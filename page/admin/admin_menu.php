        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"><i class="fa fa-edit"></i> Menu Side Bar Administration</h3>
            </div>
            <!-- /.box-header -->
              <?php
            ...
            <div class="box-body">
                <?php
                    if(!empty($_POST['submit'])){
                       if($_POST['idMenu']=='' && $_POST['submit']=="Save changes"){
                           $query = "insert into tbl_menu (nama_menu,icon_menu,url_menu,level_menu,parent_menu,type_menu,target_menu,order_menu,color_menu,authorized_roles) values(
                            '".$_POST['namaMenu']."',
                            '".$_POST['iconMenu']."',
                            '".$_POST['urlMenu']."',
                            '".$_POST['levelMenu']."',
                       ...
                       }elseif($_POST['idMenu']<>'' && $_POST['submit']=="Save changes"){
                           $query = "update tbl_menu set 
                            nama_menu='".$_POST['namaMenu']."',
                            icon_menu='".$_POST['iconMenu']."',
                            url_menu='".$_POST['urlMenu']."',
                            level_menu='".$_POST['levelMenu']."',
                            parent_menu='".$_POST['parentMenu']."',
                            type_menu='".$_POST['typeMenu']."',
                            color_menu='".$_POST['colorMenu']."',
                            order_menu='".$_POST['orderMenu']."',
                            target_menu='".$_POST['targetMenu']."',
                            authorized_roles='".$_POST['authorized']."'
                           where id_menu='".$_POST['idMenu']."'
                           ";
						   
                           $msg = "Data Menu telah diubah";
                       }elseif($_POST['idMenu']<>'' && $_POST['submit']=="Delete"){
                           $query = "delete from tbl_menu where id_menu='".$_POST['idMenu']."'
                           ";
						   $query_remove = "delete from t_perm_menu where id_menu='".$_POST['idMenu']."'
                           ";
						   db_query_insert($query_remove);
                           $msg = "Data Menu telah dihapus";
                       }
                ...
                <div class="row">
                  <div class="col-md-4">
                    <button type="button" class="btn btn-block btn-primary" onclick="addcat()">Add Menu Category</button><br>
                  </div>
                </div>
                <div class="row">
                 <div class="col-md-12" style="overflow-x:auto">
                  <table id="example1" class="table table-bordered">
                    ...
                    <tbody>
                    <?php
//                    $query = "select * from tbl_menu order by parent_menu,id_menu,level_menu";
//                    $data = db_query2list($query);
//                    $q_maxlevel = db_query("select max(level_menu) as maxLevel from tbl_menu");
//                    $maxLevel = $q_maxlevel->maxlevel;
                    //level 0
                    function showrowmenutable($fa,$color){
                        $da = 'val'.$fa;
                        global ${$da};
                        $kun = '';
                        for($i=0;$i<$fa;$i++){
                        ...
                        $txt .= "  <td>".${$da}->id_menu."</td>";
                        $txt .= "  <td>".${$da}->order_menu."</td>";
                        $txt .= "  <td>$kun ".${$da}->nama_menu."</td>";
                        $txt .= "  <td>".${$da}->icon_menu."</td>";
                        $txt .= "  <td>".${$da}->color_menu."</td>";
                        $txt .= "  <td>".${$da}->url_menu."</td>";
                        $txt .= "  <td>".${$da}->level_menu."</td>";
                        $txt .= "  <td>".${$da}->parent_menu."</td>";
                        $txt .= "  <td>".${$da}->type_menu."</td>";
                        $txt .= "  <td>".${$da}->target_menu."</td>";
                        $nextlevel = ${$da}->level_menu + 1;
                        $txt .= "  <td><a class='btn btn-primary btn-sm' onclick=\"addsub('".${$da}->id_menu."','".$nextlevel."')\">add</a> | <a  class='btn btn-success btn-sm' onclick=\"editsub('".${$da}->id_menu."','".${$da}->nama_menu."','".${$da}->icon_menu."','".${$da}->url_menu."','".${$da}->level_menu."','".${$da}->parent_menu."','".${$da}->type_menu."','".${$da}->target_menu."','".${$da}->color_menu."','".${$da}->order_menu."','".${$da}->authorized_roles."')\">edit</a> | <a  class='btn btn-danger btn-sm' onclick=\"deletesub('".${$da}->id_menu."','".${$da}->nama_menu."','".${$da}->icon_menu."','".${$da}->url_menu."','".${$da}->level_menu."','".${$da}->parent_menu."','".${$da}->type_menu."','".${$da}->target_menu."','".${$da}->color_menu."','".${$da}->order_menu."','".${$da}->authorized_roles."')\">delete</a></td>";
                        $txt .= "</tr>";
                        $kun = '';
                        return $txt;
                        
                    }
                    $query0 = "select * from tbl_menu where level_menu=0 order by order_menu";
                    $data0 = db_query2list($query0);
                    foreach($data0 as $val0){
                        echo showrowmenutable(0,'#8a959b');
                        $query1 = "select * from tbl_menu where level_menu=1 and parent_menu=$val0->id_menu order by order_menu";
                        $data1 = db_query2list($query1);
                        foreach($data1 as $val1){
                            echo showrowmenutable(1,'#a5b2ba');
                            $query2 = "select * from tbl_menu where level_menu=2 and parent_menu=$val1->id_menu order by order_menu";
                            $data2 = db_query2list($query2);
                            foreach($data2 as $val2){
                                echo showrowmenutable(2,'#b3c1c9');
                                $query3 = "select * from tbl_menu where level_menu=3 and parent_menu=$val2->id_menu order by order_menu";
                                $data3 = db_query2list($query3);
                                foreach($data3 as $val3){
                                    echo showrowmenutable(3,'#cad6df');
                                    $query4 = "select * from tbl_menu where level_menu=4 and parent_menu=$val3->id_menu order by order_menu";
                                    $data4 = db_query2list($query4);
                                    foreach($data4 as $val4){
                                        echo showrowmenutable(4,'#dbe4ee');
                                        $query5 = "select * from tbl_menu where level_menu=5 and parent_menu=$val4->id_menu order by order_menu";
                                        $data5 = db_query2list($query5);
                                        foreach($data5 as $val5){
                                            echo showrowmenutable(5, '#eef2f7');
                                        }
                                    }
                                }
                            }
...
//                    foreach($data as $val){
//                        echo "<tr>";
//                        echo "  <td>".$val->id_menu."</td>";
//                        echo "  <td>".$val->nama_menu."</td>";
//                        echo "  <td>".$val->icon_menu."</td>";
//                        echo "  <td>".$val->url_menu."</td>";
//                        echo "  <td>".$val->level_menu."</td>";
//                        echo "  <td>".$val->parent_menu."</td>";
//                        echo "  <td>".$val->type_menu."</td>";
//                        echo '  <td><a onclick="addsub('.$val->id_menu.','.$val->level_menu.')">add</a> | <a onclick="editsub('.$val->id_menu.','.$val->nama_menu.','.$val->icon_menu.','.$val->url_menu.','.$val->level_menu.','.$val->parent_menu.','.$val->type_menu.')">edit</a> | <a onclick="delsub('.$val->id_menu.')">delete</a></td>';
//                        echo "</tr>";
//                    }    
        ...
        <div class="modal modal-primary fade" id="modal-formmenu" tabindex="-1">
          <div class="modal-dialog">
            <div class="modal-content">
              <form role="form" action="adminmenu" method="post" id="menuform">
              ...
              <div class="modal-body">
                <div class="row">
                  <div class="col-md-12">
                    <input type="hidden" name="idMenu" value="" id="idMenu">
                    <div class="form-group">
                      <label for="namaMenu">Nama Menu</label>
                      <input type="text" class="form-control" name="namaMenu" id="namaMenu" placeholder="Nama Menu" required="required">
                    </div>
                    <div class="form-group col-md-4">
                      <label for="typeMenu">Type Menu</label>
                      <select class="form-control" style="width: 100%;" name="typeMenu" id="typeMenu" onchange="typechange()" required="required">
                    ...
                    <div class="form-group col-md-4">
                      ...
                      <input type="number" class="form-control" name="parentMenu" id="parentMenu" placeholder="Parent Menu" required="required">
                    </div>
                    <div class="form-group col-md-4">
                      <label for="levelMenu">Level</label>
                      <input type="number" class="form-control" name="levelMenu" id="levelMenu" placeholder="Level Menu" required="required">
                    </div>
                    <div class="form-group col-md-8">
                      <label for="urlMenu">URL</label>
                      <input type="text" class="form-control" name="urlMenu" id="urlMenu" placeholder="URL Menu" required="required">
                    </div>
                    <div class="form-group col-md-4">
                      <label for="targetMenu">Target Menu</label>
                      <select class="form-control" style="width: 100%;" name="targetMenu" id="targetMenu">
                        <option value=''>Target Menu</option>
                        <option value='_self'>Self</option>
                        <option value='_blank'>Blank</option>
                      </select>
                    </div>
                    <div class="form-group col-md-4">
                      <label for="urlMenu">Order</label>
                      <input type="number" class="form-control" name="orderMenu" id="orderMenu" placeholder="Order Menu" required="required">
                    </div>
                    <div class="form-group col-md-4">
                      <label for="targetMenu">Color Menu</label>
                      <select class="form-control" style="width: 100%;" name="colorMenu" id="colorMenu">
                        <option value=''>Color Menu</option>
                    ...
                    <div class="form-group col-md-4">
                      <label for="authorized">Authorized User</label>
                      <input type="text" class="form-control" name="authorized" id="authorized" placeholder="Authorized">
                    </div>
                    <div class="form-group col-md-6">
                      <label for="iconMenu">Icon Menu</label>
                      <select class="form-control" style="width: 100%;" name="iconMenu" id="iconMenu" required="required">
                        <option value=''></option>
                        ...
                        <option value='fa-circle-o' selected='selected'>&#xf10c; fa-circle-o</option>
...
<script>
    ...
    function addsub(idmenu,levelmenu){
        if(levelmenu > 0){
            $("#typeMenu option[value='header']").remove();
        }else{
            var o = new Option("header", "header");
            /// jquerify the DOM object 'o' so we can use the html method
            $(o).html("option text");
        ...
        $('#modal-formmenu').removeClass('modal-success').addClass('modal-primary');
        $('#modalTitle').text('Add Menu');
        var nextlevel = levelmenu;
        $('#parentMenu').val(idmenu);
        $('#levelMenu').val(nextlevel);
        $('#submitMenu').val('Save changes');
        $('#modal-formmenu').modal();
    }
    function editsub(idmenu,namamenu,iconmenu,urlmenu,levelmenu,parentmenu,typemenu,targetmenu,colormenu,ordermenu,authorized){
        $('#modal-formmenu').removeClass('modal-primary').addClass('modal-success');
        $('#menuform')[0].reset();
        $('#modalTitle').text('Edit Menu');
        $('#idMenu').val(idmenu);
        $('#namaMenu').val(namamenu);
        $('#iconMenu').val(iconmenu);
        $('#urlMenu').val(urlmenu);
        $('#levelMenu').val(levelmenu);
        $('#parentMenu').val(parentmenu);
        $('#colorMenu').val(colormenu);
        $('#orderMenu').val(ordermenu);
        $('#typeMenu').val(typemenu);
        $('#targetMenu').val(targetmenu);
        $('#authorized').val(authorized);
        $('#submitMenu').val('Save changes');
        $('#modal-formmenu').modal();
    }
    function deletesub(idmenu,namamenu,iconmenu,urlmenu,levelmenu,parentmenu,typemenu,targetmenu,colormenu,ordermenu,authorized){
        $('#modal-formmenu').removeClass('modal-primary').addClass('modal-danger');
        $('#menuform')[0].reset();
        $('#modalTitle').text('Delete Menu');
        $('#idMenu').val(idmenu);
        $('#namaMenu').val(namamenu);
        $('#iconMenu').val(iconmenu);
        $('#urlMenu').val(urlmenu);
    ...
    function typechange(){
        var menutype = $('#typeMenu').val();
        if(menutype =='header'){
            $('#levelMenu').val(0);
            $('#parentMenu').val(0);
            $('#urlMenu').attr("readonly","true");
            $('#targetMenu').attr("readonly","true");
        }else if(menutype == 'treeview'){
            $('#urlMenu').attr("readonly","true");
            $('#targetMenu').attr("readonly","true");
        }else if(menutype == 'link'){
            $('#urlMenu').removeAttr('readonly');
            $('#targetMenu').removeAttr('readonly');
        }
    }
	function cekKaryawan(){
		var cek_karyawan = $('#cek_karyawan').val();
		if(cek_karyawan =='filter'){
			$('#detail_karyawan').show();
		} else {
			$('#detail_karyawan').hide();
		}
	}
</script>